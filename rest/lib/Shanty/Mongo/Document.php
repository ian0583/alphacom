...
class Shanty_Mongo_Document extends Shanty_Mongo_Collection implements ArrayAccess, Countable, IteratorAggregate
{
	...
	protected static $_requirements = array ('_id' => 'Validator:MongoId', '_type' => 'Array' );
	...
	protected $_config = array ('new' => true, 'connectionGroup' => null, 'db' => null, 'collection' => null, 'pathToDocument' => null, 'criteria' => array (), 'parentIsDocumentSet' => false, 'requirementModifiers' => array (), 'locked' => false );
	...
	public function __construct($data = array(), $config = array())
	{
		...
		{
			static::$_collection = $this->_config ['collection'];
		}
		
		if ($this->_config ['db'])
		{
			static::$_db = $this->_config ['db'];
		}
	...
	{
		if (! $this->hasConfigAttribute ( $attribute ))
			return null;
	...
	public function getCriteria($property = null)
	{
		...
		if (! array_key_exists ( $property, $this->_config ['criteria'] ))
			return null;
	 ...
	 * If prefix is provided then only the requirements for
	 ...
	 *
	 * @param $document Shanty_Mongo_Document       	
	 * @return boolean
	 */
	 ...
	 * Save this document
	 *
	 * @param $entierDocument boolean
	 *       	 Force the saving of the entier document, instead of just the changes
	 * @param $safe boolean
	 *       	 If FALSE, the program continues executing without waiting for a database response. If TRUE, the program will wait for the database response and throw a MongoCursorException if the
	 ...
	 * Get all operations
	 *
	 * @param $includingChildren Boolean
	 *       	 Get operations from children as well
	 */
	 ...
	 *
	 * @param $includingChildren Boolean
	 *       	 Remove operations from children as wells
	 */
	...
	/*
	 * Adds value to the array only if its not in the array already. @param string $property @param mixed $value
	 */
	...
	/*
	 * Removes an element from an array @param string $property @param mixed $value
	 */
