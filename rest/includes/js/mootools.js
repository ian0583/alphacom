 ...
 * name: Browser
 * 
 * description: The Browser Object. Contains Browser initialization, Window and
 * Document, and the Browser Hash.
 * 
 * license: MIT-style license.
 * 
...
var Event = new Type(
		...
		function(event, win)
		{
			...
			if (event.$extended)
				return event;
			this.$extended = true;
			var type = event.type, target = event.target || event.srcElement, page = {}, client = {};
			...
			else if (type.test(/click|mouse|menu/i))
			{
				...
				var rightClick = (event.which == 3) || (event.button == 2), related = null;
...
/*
 * --- name: Slick.Parser description: Standalone CSS3 Selector parser provides:
 * Slick.Parser ...
 */
...
(function()
{
	...
	{// Credit: XRegExp 0.6.1 (c) 2007-2008 Steven Levithan
		// <http://stevenlevithan.com/regex/xregexp/> MIT License
	...
	pseudoClass, pseudoQuote, pseudoClassQuotedValue, pseudoClassValue)
	{
...
}).apply(/* <CommonJS> */(typeof exports != 'undefined') ? exports
		: /* </CommonJS> */this);

/*
 * --- name: Slick.Finder description: The new, superfast css selector engine.
 * provides: Slick.Finder requires: Slick.Parser ...
 */
...
(function()
{
	...
	local.setDocument = function(document)
	{
		...
		// be extrapolated, the function returns.

		if (document.nodeType === 9)
			; // document
		else if (document.ownerDocument)
		...
		var starSelectsClosed, starSelectsComments, brokenSecondClassNameGEBCN, cachedGetElementsByClassName;
		...
		if (this.isHTMLDocument)
		{
			...
			// IE returns closed nodes (EG:"</foo>") for
			// getElementsByTagName('*') for some documents
			try
			{
			...
			if (testNode.querySelectorAll)
				try
				{
					...
					this.starSelectsClosedQSA = (selected && selected.length && selected[0].nodeName
							.charAt(0) == '/');
				}
				catch (e)
				{
			...
			// getElementsById for some documents
			try
			{
			...
			// Safari 3.2 querySelectorAll doesnt work with mixedcase on
			// quirksmode
			try
			{
				testNode.innerHTML = '<a class="MiXedCaSe"></a>';
				this.brokenMixedCaseQSA = !testNode
						.querySelectorAll('.MiXedCaSe').length;
			}
			catch (e)
			{
			}
			...
			this.brokenGEBCN = cachedGetElementsByClassName
			...
			// IE returns incorrect results for attr[*^$]="" selectors on
			// querySelectorAll
			try
			{
				testNode.innerHTML = '<a class=""></a>';
				this.brokenEmptyAttributeQSA = (testNode
		...
		this.hasAttribute = (root && this.isNativeCode(root.hasAttribute)) ? function(
				node, attribute)
		{
		...
		// contains
		// FIXME: Add specs: local.contains should be different for xml and html
		// documents?
		this.contains = (root && this.isNativeCode(root.contains)) ? function(
				context, node)
		{
	...
	local.search = function(context, expression, append, first)
	{
		...
		if (!context)
			return found; // No context
		if (context.navigator)
			context = context.document; // Convert the node from a window to a
										// document
		else if (!context.nodeType)
			return found; // Reject misc junk input
		...
		if (this.document !== (context.ownerDocument || context))
		...
		if (shouldUniques)
			for (i = found.length; i--;)
				this.uniques[this.getUID(found[i])] = true;
		...
		{ // expression is a string
			...
			for (i = this.overrides.length; i--;)
			{
				...
				{
					var result = override.method.call(context, expression,
							found, first);
					if (result === false)
						continue;
					if (result === true)
		...
		// expression use pushArray, else use pushUID
		this.push = (!shouldUniques && (first || (parsed.length == 1 && parsed.expressions[0].length == 1))) ? this.pushArray
				: this.pushUID;
		...
		if (shouldUniques || (parsed.expressions.length > 1))
			this.sort(found);
	...
	local.matchSelector = function(node, tag, id, classes, attributes, pseudos)
	{
		...
		if (classes)
			...
			{
				cls = ('className' in node) ? node.className : node
						.getAttribute('class');
		...
		{ // all child nodes, any level
			...
			if (this.isHTMLDocument)
			{
				getById: if (id)
				{
					...
					{
						// all[id] returns all the elements with that name or id
						// inside node
						// if theres just one it will return the element, else
						...
						for (i = 0; item = children[i++];)
							if (item.getAttributeNode('id').nodeValue == id)
							{
					...
					if (!item)
					{
						// if the context is in the dom we return, else we will
						// try GEBTN, breaking the getById label
						if (this.contains(this.document.documentElement, node))
							return;
						else
							break getById;
					}
	...
	local.attributeGetters = {
		...
		'class' : function()
		{
			...
			return ('className' in this) ? this.className : this
					.getAttribute('class');
		},
	...
	local.override(/./,
			function(expression, found, first)
			{ // querySelectorAll override

				if (!this.querySelectorAll
						|| this.nodeType != 9
						|| !local.isHTMLDocument
						|| local.brokenMixedCaseQSA
						|| (local.brokenCheckedQSA && expression
								.indexOf(':checked') > -1)
						|| (local.brokenEmptyAttributeQSA && reEmptyAttribute
								.test(expression)) || Slick.disableQSA)
					return false;
				...
				{
					if (first)
						return this.querySelector(expression) || null;
					else
						nodes = this.querySelectorAll(expression);
				}
				catch (error)
				{
					return false;
				...
				if (local.starSelectsClosedQSA)
					...
					{
						if (!hasOthers
								|| !local.uniques[local.getUIDHTML(node)])
							found.push(node);
					}
	...
	{ // tag override
		...
		for (i = 0; node = nodes[i++];)
		{
			if (!hasOthers || !local.uniques[local.getUID(node)])
				found.push(node);
		}
	...
	local
			.override(
					...
					{ // class override
						...
						if (!local.isHTMLDocument
								|| (!this.getElementsByClassName && this.querySelectorAll))
							return false;

						var nodes, node, i, hasOthers = !!(found && found.length), className = expression
								.substring(1);
						...
						else
						{
							var matchClass = new RegExp('(^|\\s)'
									+ Slick.escapeRegExp(className) + '(\\s|$)');
	...
	{ // ID override
		...
		if (!local.isHTMLDocument || this.nodeType != 9)
			return false;
		...
		if (!hasOthers || !local.uniques[local.getUIDHTML(el)])
			found.push(el);
		if (hasOthers)
			local.sort(found);
		return true;
 ...
 * name: Element
 * 
 * description: One of the most important items in MooTools. Contains the dollar
 * function, the dollars function, and an handful of cross-browser, time-saver
 * methods to let you easily work with HTML Elements.
 * 
 * license: MIT-style license.
 * 
	...
	Element
			.implement({
				...
				clone : function(contents, keepid)
				{
					...
					if (Browser.ie)
					{
						var ts = this.getElementsByTagName('object'), cs = clone
								.getElementsByTagName('object'), tl = ts.length, cl = cs.length;
						for (i = 0; i < tl && i < cl; i++)
 ...
 * name: Element.Style
 * 
 * description: Contains methods for interacting with the styles of Elements in
 * a fashionable way.
 * 
 * license: MIT-style license.
 * 
...
(function()
{
	...
	var setOpacity = function(element, opacity)
	{
		...
		else
		{
			...
			element.style.filter = filter.test(reAlpha) ? filter.replace(
 ...
 * name: Element.Event
 * 
 * description: Contains Element methods for dealing with events. This file also
 * includes mouseenter and mouseleave custom Element Events.
 * 
 * license: MIT-style license.
 * 
					...
					'implement',
					{
						...
						removeEvent : function(type, fn)
						{
							...
							return (Element.NativeEvents[type]) ? this
									.removeListener(type, value) : this;
						},
 ...
 * 
 * description: Contains methods to work with size, scroll, or positioning of
 * Elements and the window object.
 * 
 * license: MIT-style license.
 * 
 * credits: - Element positioning based on the [qooxdoo](http://qooxdoo.org/)
 * code and smart browser fixes, [LGPL
 * License](http://www.gnu.org/licenses/lgpl.html). - Viewport dimensions based
 * on [YUI](http://developer.yahoo.com/yui/) code, [BSD
 ...
 * name: Fx
 * 
 * description: Contains the basic animation logic to be extended by all other
 * Fx Classes.
 * 
 * license: MIT-style license.
 * 
...
(function()
{
	...
	var addInstance = function(instance)
	{
		...
		var fps = instance.options.fps, list = instances[fps]
				|| (instances[fps] = []);
		list.push(instance);
		if (!timers[fps])
 ...
 * name: Fx.CSS
 * 
 * description: Contains the CSS animation logic. Used by Fx.Tween, Fx.Morph,
 * Fx.Elements.
 * 
 * license: MIT-style license.
 * 
 ...
 * name: Fx.Tween
 * 
 * description: Formerly Fx.Style, effect to transition any CSS property for an
 * element.
 * 
 * license: MIT-style license.
 * 
			...
			fade : function(how)
			{
				...
				switch (how)
				{
					...
					case 'hide':
						...
						break;
					case 'toggle':
						var flag = this.retrieve('fade:flag', this
								.get('opacity') == 1);
 ...
 * properties for an element using an object of rules, or CSS based selector
 * rules.
 * 
 * license: MIT-style license.
 * 
 ...
 * name: Fx.Transitions
 * 
 * description: Contains a set of advanced transitions to be used with any of
 * the Fx Classes.
 * 
 * license: MIT-style license.
 * 
 * credits: - Easing Equations by Robert Penner,
 * <http://www.robertpenner.com/easing/>, modified and optimized to be used with
 * MooTools.
 * 
 * requires: Fx
 * 
	...
	var Request = this.Request = new Class(
			{
				...
				options : {/*
							 * onRequest: function(){}, onLoadstart:
							 * function(event, xhr){}, onProgress:
							 * function(event, xhr){}, onComplete: function(){},
							 * onCancel: function(){}, onSuccess:
							 * function(responseText, responseXML){}, onFailure:
							 * function(xhr){}, onException:
							 * function(headerName, value){}, onTimeout:
 ...
 * interacting with HTML responses.
 * 
 * license: MIT-style license.
 * 
...
(function(window, document)
{
	...
	var ready, loaded, checks = [], shouldPoll, timer, isFramed = true;
 ...
 * name: Swiff
 * 
 * description: Wrapper for embedding SWF movies. Supports External Interface
 * Communication.
 * 
 * license: MIT-style license.
 * 
 * credits: - Flash detection & Internet Explorer + Flash Player 9 fix inspired
 * by SWFObject.
 * 
