...
class lessc
{
	...
	protected static $precedence = array( '=<' => 0, '>=' => 0, '=' => 0, '<' => 0, '>' => 0, 
	...
	protected static $dtypes = array( 'expression', 'variable', 'function', 'negative', 'list', 'lookup' );
	 ...
	 */
	protected static $units = array( 'em', 'ex', 'px', 'gd', 'rem', 'vw', 'vh', 'vm', 'ch', 	// Relative
	                                                                                        // length
	                                                                                        // units
	...
	function parseChunk ()
	{
		...
		if ( $this->import( $url, $media ) )
		{
			...
			$this->append( array( 'raw', '@import url("' . $url . '")' . ( $media ? ' ' . $media : '' ) . ';' ), $s );
			return true;
		}
		...
		// mixin
		if ( $this->mixinTags( $tags ) && ( $this->argumentValues( $argv ) || true ) && $this->end() )
		{
			$tags = $this->fixTags( $tags );
	...
	function mediaTypes ( &$parts )
	{
		...
		while ( $this->to( "(", $chunk, false, "[^{]" ) )
		{
			...
			$s = $this->seek();
			if ( $this->keyword( $name ) && $this->assign() && $this->propertyValue( $value ) )
			{
	...
	{
		$s = $this->seek();
		if ( $this->literal( $this->vPrefix, false ) && ( $this->variable( $sub ) || $this->keyword( $name ) ) )
		{
			if ( ! empty( $sub ) )
			{
	...
	function compileBlock ( $block, $parent_tags = null )
	{
		...
		if ( $special_block )
		{
			...
			{
				echo $block->tags[ 0 ] . " " . $this->compileValue( $this->reduce( $block->keyframes ) );
			}
			else
			{
	...
	function findBlocks ( $search_in, $path, $args, $seen = array() )
	{
		...
		if ( isset( $search_in->children[ $name ] ) )
		{
			...
			{
				return $this->findBlocks( $blocks[ 0 ], array_slice( $path, 1 ), $args, $seen );
			}
		}
	...
	function compileProp ( $prop, $block, $tags, &$_lines, &$_blocks )
	{
		...
		switch ( $prop[ 0 ] )
		{
			case 'assign':
				...
				{
					$_lines[] = "$name:" . $this->compileValue( $this->reduce( $value ) ) . ";";
				}
				break;
			case 'block':
				list ( , $child ) = $prop;
	...
	function compileValue ( $value )
	{
		switch ( $value[ 0 ] )
		{
			...
			case 'function':
				// [1] - function name
				// [2] - some array value representing arguments, either ['string', value] or
				// ['list', ',', values[]]
	...
	function compileMedia ( $block )
	{
		...
		foreach ( $block->media as $part )
		{
			...
			elseif ( $part[ 0 ] == "assign" )
			{
				...
				$mediaParts[] = "$propName: " . $this->compileValue( $this->reduce( $propVal ) );
			}
		}
	...
	function reduce ( $var )
	{
		...
		if ( $var[ 0 ] == "variable" )
		{
			$key = is_array( $var[ 1 ] ) ? $this->vPrefix . $this->toName( $this->reduce( $var[ 1 ] ) ) : $var[ 1 ];
	...
	public static function ccompile ( $in, $out, $force = true )
	{
	...
	/**
	 * Execute lessphp on a .
	 *
	 *
	 * less file or a lessphp cache structure
	 *
