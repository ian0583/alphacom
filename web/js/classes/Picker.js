/*
 * --- name: Picker description: Creates a Picker, which can be used for
 * anything authors: Arian Stolwijk requires: [Core/Element.Dimensions,
 * Core/Fx.Tween, Core/Fx.Transitions] provides: Picker ...
 */
...
var Picker = new Class(
	...
	options :
		{/*
			 * onShow: function(){}, onOpen: function(){}, onHide: function(){},
			 * onClose: function(){},
			 */
	...
	constructPicker : function()
	{
		...
		// oldContents and newContents are used to slide from the old content to
		// a new one.
	...
	{
		var offset = this.options.positionOffset, scroll = document.getScroll(), size = document.getSize(), pickersize = this.picker.getSize();
		...
		if ( ( x + pickersize.x ) > ( size.x + scroll.x ) )
			x = ( size.x + scroll.x ) - pickersize.x;
		if ( ( y + pickersize.y ) > ( size.y + scroll.y ) )
			y = ( size.y + scroll.y ) - pickersize.y;
		if ( x < 0 )
			x = 0;
		if ( y < 0 )
			y = 0;
	...
	setColumnContent : function( column, content )
	{
		...
		if (
			[ 'string', 'number' ].contains( type ) )
			columnElement.set( 'text', content );
		else
	...
	fx : function( fx )
	{
		var oldContents = this.oldContents, newContents = this.newContents, slider = this.slider, bodysize = this.bodysize;
		if ( fx == 'right' )
		{
	...
	setTitleEvent : function( fn )
	{
		...
		if ( fn )
			this.titleText.addEvent( 'click', fn );
