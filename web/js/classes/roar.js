...
var Roar = new Class(
	...
	remove : function( item )
	{
		...
		return this.fireEvent( 'onHide',
			[ item, this.items.length ] ).callChain( item );
	},
	...
	reposition : function()
	{
		...
		this.moveTo(
			{
			left : ( this.position.x == 'right' ) ? ( Math.min( rel.right, max.right ) - margin.x ) : ( Math.max( rel.left, max.left ) + margin.x ),
			top : ( this.position.y == 'bottom' ) ? ( Math.min( rel.bottom, max.bottom ) - margin.y ) : ( Math.max( rel.top, max.top ) + margin.y )
			} );
	}
