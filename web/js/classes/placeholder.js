 ...
 * name: mooPlaceholder
 * 
 * description: provides placeholder= behaviour to browsers that don't support
 * it.
 * 
 ...
 * version: 1.5
 * 
 * requires: - Core/String - Core/Event - Core/Element - Core/Array - Core/Class
 * 
...
( function()
{
	...
	var mooPlaceholder = this.mooPlaceholder = new Class(
		...
		options :
			{
			// default options
			htmlPlaceholder : "placeholder", // the element attribute, eg,
			// data-placeholder="MM/YY" ->
			// "data-placeholder"
			unmoddedClass : "unchanged", // apply a class to the unmodded
			// input, say, to grey it out
		...
		{
			// reset managed fields values. call this on form submit before
			// validation!
		...
		change : function( el )
		{
			...
			{
				// once it changes, remove this check and remove the
				// unmoddedClass
...
{
	globalPlaceholder = new mooPlaceholder();
	
	globalPlaceholder.attachToElements()
